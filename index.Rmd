...
.column.bg-main3[.content.vmiddle.center[
## This tells the `lm` function what variables we would like to regress. R expects the relationship in the form of `response~predictors`. 
]]
...
Can we use a simple linear model to predict the presence of a spider a new data point?
...
ggplot(data, aes(x=GrainSize, y=Spiders))+geom_point(size=3, color="red")
```
...
```{r, fig.retina=4, echo=FALSE}
ggplot(data, aes(x=Exam1, y=Exam2, color=factor(Label)))+geom_point(size=4)
```
